...
# python train.py --vit_model ViT-B-32 --batch_size 32 --learning_rate 1e-4 --epochs 2 --save_dir "./saved_models_RN-18" --visualize_features --vis_interval 1
...
def visualize_features(features, labels, text_descriptions, epoch, save_dir, prefix=""):
    """使用t-SNE可视化特征"""
...
def zero_shot_evaluation(model, text_descriptions, data_loader, device):
    """零样本分类评估"""
    model.eval()
    # 获取文本特征
    with torch.no_grad():
...
def train(args):
    ...
    # 使用数据集中已经处理好的文本描述
    text_descriptions = dataset.label_texts
    ...
    # 训练前进行零样本分类评估
    print("训练前进行零样本分类评估...")
    initial_accuracy = zero_shot_evaluation(model, text_descriptions, val_loader, device)
    # 记录最佳零样本准确率
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.vis_interval == 0 and args.visualize_features:
            ...
            with torch.no_grad():  # 不计算梯度，节省内存
                # 只使用少量样本
                sample_count = 0
                max_samples = 1000  # 最大样本数，GPU内存占用2000->3.3G
            ...
            # 可视化
            visualize_features(all_vit_features, all_labels, text_descriptions, epoch + 1, args.save_dir, "ViT")
            visualize_features(all_resnet_features, all_labels, text_descriptions, epoch + 1, args.save_dir, "ResNet")
            visualize_features(all_cross_attn_features, all_labels, text_descriptions, epoch + 1, args.save_dir, "CrossAttn")
        ...
        zero_shot_acc = zero_shot_evaluation(model, text_descriptions, val_loader, device)
