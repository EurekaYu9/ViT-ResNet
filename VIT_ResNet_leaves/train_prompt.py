...
def train(args):
    ...
    # 训练前进行零样本分类评估
    print("训练前进行零样本分类评估...")
    initial_accuracy = zero_shot_evaluation(model, class_names, val_loader, device, prompts)
    # 记录最佳零样本准确率
    ...
    for epoch in range(args.epochs):
        ...
        zero_shot_acc= zero_shot_evaluation(model, class_names, val_loader, device, prompts)
    ...
    # 训练结束后的最终评估
    print("\n进行最终零样本分类评估...")
    final_accuracy = zero_shot_evaluation(model, class_names, val_loader, device, prompts)
