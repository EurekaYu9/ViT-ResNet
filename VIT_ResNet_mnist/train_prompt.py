...
# python train.py --vit_model ViT-B-32 --resnet_model RN50 --batch_size 32 --learning_rate 1e-4 --epochs 1 --save_dir "./saved_models" --visualize_features --vis_interval 1
...
def visualize_features(features, labels, epoch, save_dir, prefix=""):
    """使用t-SNE可视化特征"""
    ...
    plt.title(f"{prefix} Features Space Visualization (Epoch {epoch})")
...
def zero_shot_evaluation(model, data_loader, device):
    ...
    text_descriptions = [
        "A photo of handwritten digit 0",
        "A photo of handwritten digit 1",
        "A photo of handwritten digit 2",
        "A photo of handwritten digit 3",
        "A photo of handwritten digit 4",
        "A photo of handwritten digit 5",
        "A photo of handwritten digit 6",
        "A photo of handwritten digit 7",
        "A photo of handwritten digit 8",
        "A photo of handwritten digit 9"
    ]
...
def train(args):
    ...
    # 在训练前进行零样本评估
    print("训练前进行零样本分类评估...")
    initial_accuracy = zero_shot_evaluation(model, val_loader, device)
    # 记录最佳零样本准确率
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.vis_interval == 0 and args.visualize_features:
            ...
            with torch.no_grad():  # 不计算梯度，节省内存
                # 只使用少量样本
                sample_count = 0
                max_samples = 2000  # 最大样本数，GPU内存占用2000->3.3G
            ...
            # 可视化
            visualize_features(all_vit_features, all_labels, epoch + 1, args.save_dir, "ViT")
            visualize_features(all_resnet_features, all_labels, epoch + 1, args.save_dir, "ResNet")
            visualize_features(all_cross_attn_features, all_labels, epoch + 1, args.save_dir, "CrossAttn")
        ...
        zero_shot_acc = zero_shot_evaluation(model, val_loader, device)
        ...
        if zero_shot_acc > best_zero_shot_acc:
            ...
            torch.save(model.state_dict(), os.path.join(args.save_dir, 'best_hybrid_model.pth'))
            print(f"!!!最佳混合注意力模型已保存，准确率: {best_zero_shot_acc:.2f}%!!!")
    
    # 训练结束后的最终评估
    print("\n进行最终零样本分类评估...")
    final_accuracy = zero_shot_evaluation(model, val_loader, device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="训练混合注意力模型-MNIST")
